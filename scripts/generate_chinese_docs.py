...
- `docs/references/recommended_reading_order_zh.md`：28 篇论文推荐阅读顺序。
...
- `recommended_reading_order_zh.md`：覆盖 28 篇论文的详细中文阅读顺序。
...
本文整合 `docs/references/papers/` 下 28 篇论文，目标不是平均总结每篇论文，而是把文献转化为 2RL 项目的开发路线。
...
## 8. VLA 与 Diffusion 拓展动作生成视角

DDPM、DDIM、ACT、Diffusion Policy 和 pi0 把阅读范围从 locomotion 扩展到 diffusion foundation、vision-language-action 和 visuomotor manipulation。它们不是当前 2RL 第一复现目标，但能帮助理解现代机器人策略如何输出一段动作序列，而不是每一步独立回归动作。

推荐理解顺序：

1. DDPM：理解扩散模型的去噪训练目标；
2. DDIM：理解快速采样和延迟/质量权衡；
3. ACT：用 transformer 做 action chunking；
4. Diffusion Policy：用 diffusion 生成连续动作序列；
5. pi0：把 VLM 条件、flow matching 和机器人 foundation policy 连接起来。
...
9. Humanoid 系列。
10. VLA/diffusion 拓展：DDPM、DDIM、ACT、Diffusion Policy、pi0。
...
- 读 DDPM，理解 diffusion 去噪训练目标。
- 读 DDIM，理解快速采样和延迟/质量权衡。
